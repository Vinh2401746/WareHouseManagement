...
import { getInvoiceByIdApi, updateInvoiceApi } from "../../../api/sales";
    ...
    () => {
        ...
        useEffect(() => {
            ...
            if (paramDetail?.id) {
                const fetchData = async () => {
                    try {
                        const response: any = await getInvoiceByIdApi({ id: paramDetail.id })
                        if (response.id) {
        ...
        const { mutate } = useMutation({
            mutationFn: (payload: any) => {
                return updateInvoiceApi({ data: payload.data, id: dataImport?.id || null });
            },
            onSuccess: () => {
                dispatchToast(
                    "success",
        ...
        const onFinish = (values: WarehouseImExFormData) => {
            ...
            if (dataImport?.status === 'COMPLETED') return dispatchToast("warning", "Đơn đã hoàn thành nên không thể cập nhật");
        ...
        return (
            <div>
                ...
                <Form
                    ...
                    disabled={dataImport?.status === 'COMPLETED'}
                >
                    <Row gutter={8}>
                        <Col span={12}>
                            <Form.Item
